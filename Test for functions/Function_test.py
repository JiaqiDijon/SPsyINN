...
class DSR(nn.Module):
    ...
    def test(self, x, name):
        ...
        if name == 'En2De':
            ...
            x5 = x[:, :, 4]
            x6 = x[:, :, 5]
            product_term = x2 * x3 * x5 * x6
            exp_term = torch.exp(product_term - x2)
            result = torch.sin(x2 + exp_term)
        ...
        if name == 'En2Es':
            ...
            x5 = x[:, :, 4]
            x6 = x[:, :, 5]
            product_term = torch.exp(-x5 + x6)
            exp_term = torch.exp(-x3 + x6 - product_term)
            result = torch.cos(exp_term)
