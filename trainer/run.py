...
if os_type == "Windows":
    ...
    process2 = subprocess.Popen(["python", "GSR_train.py"], creationflags=subprocess.CREATE_NEW_CONSOLE)

elif os_type == "Linux":
    print("Detected Linux OS.")
    process1 = subprocess.Popen(["xterm", "-e", "python TorchModel_train.py"])
    process2 = subprocess.Popen(["xterm", "-e", "python GSR_train.py"])

elif os_type == "Darwin":  # macOS
    print("Detected macOS.")
    process1 = subprocess.Popen(["open", "-a", "Terminal", "--args", "python TorchModel_train.py"])
    process2 = subprocess.Popen(["open", "-a", "Terminal", "--args", "python GSR_train.py"])
